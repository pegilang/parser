 ...
 */
public abstract class PegiParserBase extends Parser
{
    protected PegiParserBase(TokenStream input) {
        super(input);
    }
    ...
    protected boolean lineTerminatorAhead() {
        ...
        {
            if(ahead.getType() == PegiLexer.TERMINATOR){
                return true;
            }
            else if(ahead.getType() == PegiLexer.WS){
                possibleIndexEosToken = this.getCurrentToken().getTokenIndex() - ++offset;
                ahead = _input.get(possibleIndexEosToken);
            }
            else if(ahead.getType() == PegiLexer.COMMENT || ahead.getType() == PegiLexer.LINE_COMMENT ){
    ...
    protected boolean noTerminatorAfterParams(int tokenOffset) {
        ...
        if (stream.LT(tokenOffset).getType() == PegiLexer.L_PAREN) {
            // Scan past parameters
            while (leftParams != rightParams) {
                tokenOffset++;
                int valueType = stream.LT(tokenOffset).getType();

                if (valueType == PegiLexer.L_PAREN){
                    leftParams++;
                }
                else if (valueType == PegiLexer.R_PAREN) {
                    rightParams++;
                }
            }
